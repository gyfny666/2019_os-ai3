...
void Allocation(pLNode L, int len, int num) {
	...
	if (minplace!=INF&&bestP->length > len) {
		//表示可以分配内存空间
		//但是需要把内部碎片分出来，构成新的空闲内存块
		//新的内存块的起始地址为bestP->address + len，长度为p->length-len
		pLNode l = (pLNode)malloc(sizeof(LNode));
		l->length = bestP->length - len;
		l->address = bestP->address + len;
		l->state = 0;
	...
	else if (minplace!=INF&&bestP->length == len) {
